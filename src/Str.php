...
class Str
{
    ...
    public static function afterFirst(string $string, string $substring): string
    {
        ...
        $position = grapheme_strpos($string, $substring);
    ...
    public static function afterLast(string $string, string $substring): string
    {
        ...
        $position = grapheme_strrpos($string, $substring);
    ...
    public static function betweenFirst(string $string, string $from, string $to): string
    {
        ...
        $endPos = grapheme_strpos($string, $to, $startPos);
        if ($endPos === false) {
            throw new RuntimeException("\$to: \"$to\" does not exist after \$from in \"$string\"");
        }
    ...
    public static function betweenLast(string $string, string $from, string $to): string
    {
        ...
        $startPos = grapheme_strrpos($string, $from);
        if ($startPos === false) {
            throw new RuntimeException("\$from: \"$from\" does not exist in \"$string\"");
        }
        $startPos += static::length($from);
    ...
    public static function count(string $string, string $substring): int
    {
        ...
        while ($offset < $length) {
            $position = grapheme_strpos($string, $substring, $offset);
     ...
     *
     * @param string $string
     * If you want to know the bytes used for a given string instead,
    ...
    ): string
    {
        ...
        $offset = 0;
        while (($position = grapheme_strpos($replaced, $search, $offset)) !== false && $max > $count) {
    ...
    ): string
    {
        ...
        foreach ($searches as $search) {
            ...
            $position = grapheme_strpos($replaced, $search, $offset);
            if ($position === false) {
                continue;
            }
            $before = static::substring($replaced, 0, $position);
    ...
    ): string
    {
        ...
        $position = grapheme_strrpos($string, $search);
